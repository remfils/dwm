...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMWindowTypeDialog, NetClientList, NetNumberOfDesktops, NetCurrentDesktop, NetDesktopNames, NetLast }; /* EWMH atoms */
...
scan(void)
{
	...
	if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
		for (i = 0; i < num; i++) {
			fprintf(stderr, "testing value %d, override: %d, usealtbar: %d\n", i, wa.override_redirect, usealtbar);
			if (!XGetWindowAttributes(dpy, wins[i], &wa)
                            || wa.override_redirect || XGetTransientForHint(dpy, wins[i], &d1)) {
                          continue;
                        }
			fprintf(stderr, "\t map state %d, getstate: %d\n", wa.map_state == IsViewable, getstate(wins[i]) == IconicState);
...
wmclasscontains(Window win, const char *class, const char *name)
{
	...
	if (XGetClassHint(dpy, win, &ch)) {
                fprintf(stderr, "wmclasscontains: name: %s class %s\n", ch.res_name, ch.res_class);
		...
		if (ch.res_class && strstr(ch.res_class, class) == NULL)
          ...
          if (gettextprop(win, netatom[NetWMName], tempString, 120)) {
            fprintf(stderr, "found string %s\n", tempString);
            
            if (strstr(tempString, class) != NULL) {
              res = 1;
            } else {
              fprintf(stderr, "no class hint\n");
              res = 0;
            }
          } else {
            fprintf(stderr, "no class hint\n");
